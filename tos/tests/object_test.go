...
func TestGetFileStatus(t *testing.T) {
	...
	_, err = client.AppendObjectV2(context.Background(), &tos.AppendObjectV2Input{
		Bucket:  bucket,
		Key:     key,
		Content: strings.NewReader(randomString(1024 * 8)),
	})
	require.Nil(t, err)
	resp, err = client.GetFileStatus(ctx, &tos.GetFileStatusInput{Bucket: bucket, Key: key})
	require.Nil(t, err)
...
func TestDocPreview(t *testing.T) {
	...
	client, err := tos.NewClientV2(env.endpoint2, options...)
	require.Nil(t, err)
	_, err = client.CreateBucketV2(context.Background(), &tos.CreateBucketV2Input{Bucket: bucket})
	require.Nil(t, err)
...
func TestObjectWithStsRetry(t *testing.T) {
	var (
		env    = newTestEnv(t)
		bucket = generateBucketName("object-sts-with-retry")
		key    = randomString(6)
		ctx    = context.Background()
	)
	tsConfig := tos.DefaultTransportConfig()
	...
	client := env.prepareClient(bucket, tos.WithTransportConfig(&tsConfig), tos.WithMaxRetryCount(5))
	defer cleanBucket(t, client, bucket)
	resp, err := assumeRole(env.topEndpoint, env.region, env.roleName, env.accountId, env.accessKey, env.secretKey)
	require.Nil(t, err)
	require.NotNil(t, resp)
	cred := tos.NewStaticCredentials(resp.Result.Credentials.AccessKeyId, resp.Result.Credentials.SecretAccessKey)
	cred.WithSecurityToken(resp.Result.Credentials.SessionToken)
	client, err = tos.NewClientV2(env.endpoint, tos.WithCredentials(cred), tos.WithRegion(env.region), tos.WithTransportConfig(&tsConfig), tos.WithMaxRetryCount(5))
	require.Nil(t, err)
	// Case 1: 测试内存流
	rawData := "hello world"
	dataListener := &dataTransferListenerTest{}
...
func TestDoseObjectExist(t *testing.T) {
	var (
		env    = newTestEnv(t)
		bucket = generateBucketName("exist-bucket")
		client = env.prepareClient(bucket)
		key    = "exist-key"
	)
	defer func() {
	...
	client.PutObjectV2(context.Background(), &tos.PutObjectV2Input{
		PutObjectBasicInput: tos.PutObjectBasicInput{Bucket: bucket, Key: key},
		Content:             strings.NewReader("test"),
	})

	res, err := client.DoesObjectExist(context.Background(), &tos.DoesObjectExistInput{Bucket: bucket, Key: key})
	require.Nil(t, err)
	require.True(t, res)

	res, err = client.DoesObjectExist(context.Background(), &tos.DoesObjectExistInput{Bucket: bucket, Key: "none-exist-key"})
	require.Nil(t, err)
	require.False(t, res)
}
