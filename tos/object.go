...
func (cli *ClientV2) VideoDataProcess(ctx context.Context, input *VideoDataProcessInput) (*VideoDataProcessOutput, error) {
	if input == nil {
		return nil, InputIsNilClientError
	}
	if err := isValidBucketName(input.Bucket, cli.isCustomDomain); err != nil {
	...
	res, err := cli.newBuilder(input.Bucket, input.Key).
		...
		Request(ctx, http.MethodPost, bytes.NewReader([]byte(process)), cli.roundTripper(http.StatusOK))
	if err != nil {
		return nil, err
	}
	defer res.Close()

	output := VideoDataProcessOutput{
		RequestInfo: res.RequestInfo(),
	}
	if err = marshalOutput(res, &output); err != nil {
		return nil, err
